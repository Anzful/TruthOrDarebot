...
@bot.command()
async def truth(ctx):
  embed = discord.Embed(title=f"{ctx.author.name}, {random.choice(truth_norm)}", colour=discord.Colour.dark_gold())

  await ctx.send(embed=embed)

@bot.command()
async def dare(ctx):
  embed = discord.Embed(title=f"{ctx.author.name}, {random.choice(dare_norm)}", colour=discord.Colour.dark_orange())
